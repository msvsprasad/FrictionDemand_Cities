...
for index_sectionID = 11%1:numel(list_of_sectionIds)
    if list_of_sectionIds(index_sectionID) > 0
        disp(index_sectionID)
        if flag.dbQuery
            disp('Query for section ID vehicle trajectory')
            raw_trajectory = fcn_queryVehicleTrajectory(list_of_sectionIds(index_sectionID),...
        ...
        index_vehicle = unique(raw_trajectory{:,{'vehicle_id'}});
    ...
    end % NOTE: END IF statement to only query data for sections, NOT junctions
end
