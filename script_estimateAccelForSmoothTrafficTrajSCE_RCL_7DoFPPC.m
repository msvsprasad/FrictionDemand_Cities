...
% test_list_of_sectionIds = readmatrix('test_section_ids.xlsx');
for index_sectionID = 454:numel(list_of_sectionIds)
    if list_of_sectionIds(index_sectionID) > 0
        ...
        if isempty(cg_east_VT) == 0
            ...
            [LCL_lane1, LCL_lane2, LCL_lane3, LCL_lane4, LCL_XY_all] = ...
            ...
            for index_vehicle = 1:length(vehicleIds_on_sectionId)
                ...
                vehicle_trajectory = raw_trajectory(index_raw_path,:);
                if length(vehicle_path_XY(:,1)) > 1
                    % run a knn search to find which lane you're on: input, specific vehicle trajectory
                    %   and every lane trajectory
                    % X and Y are all of the LCL in one column
                    Idx = knnsearch([LCL_XY_all(:,1),LCL_XY_all(:,2)],...
                        [vehicle_path_XY(:,1),vehicle_path_XY(:,2)]);
                    % based on the nearest index find the corresponding nearest lane
                    % use the Idx value to find the yaw LCL that corresponds to each trajectory
                    %   point,
                    closest_lane_number = floor((Idx-1)/length(RCL_path))+1;
                    % run a for loop with the snap function
                    vehicle_yaw = [];
                    for index_vehicle_path_point = 1:length(vehicle_path_XY)
                        % figure out what lane corresponds to the index point
                        %   thats the traversal input
                        vehicle_path_point = vehicle_path_XY(index_vehicle_path_point,[1,2]);
                        lane_number = closest_lane_number(index_vehicle_path_point);
                        if lane_number == 1
                            traversal = LCL_lane1;
                        ...
                        [closest_path_point,s_coordinate,path_point_yaw,....
                            ...
                            fcn_Path_snapPointOntoNearestTraversal(vehicle_path_point, traversal);

                        vehicle_yaw = [vehicle_yaw; path_point_yaw];
                    end
                    ...
                    for index_stop = 1:number_of_stops
                        ...
                        start_stop_trajectory = vehicle_trajectory(indices_to_start(index_stop):...
                        ...
                        if size(start_stop_trajectory,1) > min_trip_size
                            ...
                            counter = 1;
                            friction_demand_fl_total = [];
                            friction_demand_fr_total = [];
                            friction_demand_rl_total = [];
                            friction_demand_rr_total = [];
                            ...
                            for t = 0:deltaT:TotalTime
                                ...
                                % store tire friction forces
                                tire_forces_fl = sqrt(tire_forces(1,1)^2 + tire_forces(1,2)^2);
                                tire_forces_fr = sqrt(tire_forces(2,1)^2 + tire_forces(2,2)^2);
                                tire_forces_rl = sqrt(tire_forces(3,1)^2 + tire_forces(3,2)^2);
                                tire_forces_rr = sqrt(tire_forces(4,1)^2 + tire_forces(4,2)^2);

                                friction_demand_fl = tire_forces_fl/normal_force(1,1);
                                friction_demand_fr = tire_forces_fr/normal_force(2,1);
                                friction_demand_rl = tire_forces_rl/normal_force(3,1);
                                friction_demand_rr = tire_forces_rr/normal_force(4,1);

                                friction_demand_fl_total = [friction_demand_fl_total; friction_demand_fl];
                                friction_demand_fr_total = [friction_demand_fr_total; friction_demand_fr];
                                friction_demand_rl_total = [friction_demand_rl_total; friction_demand_rl];
                                friction_demand_rr_total = [friction_demand_rr_total; friction_demand_rr];
                            ...
                            %% Plot the results
                            figure(3)
                            plot(vehicle_path_XY(:,1),vehicle_path_XY(:,2),'b.')
                            ...
                            if flag.plot
                                ...
                                fcn_VD_plotStationFrictionDemand(cg_station,friction_demand,08)
                            ...
                            if flag.dbInsert
                                ...
                                friction_measurement.vehicle_id = ...
                                    vehicleIds_on_sectionId(index_vehicle)*ones(output_data_length,1);
                                ...
                                [cg_new,ia,~] = unique([cg_east_VT,cg_north_VT],'rows');
                                ...
                                % Force Demand
                                friction_measurement.friction_true_fl = friction_demand_fl_total;
                                friction_measurement.friction_true_fr = friction_demand_fr_total;
                                friction_measurement.friction_true_rl = friction_demand_rl_total;
                                friction_measurement.friction_true_rr = friction_demand_rr_total;
