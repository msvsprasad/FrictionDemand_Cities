...
%             vehicle_yaw_interp];
    ...
    elseif list_of_sectionIDs_traj_1(i) == -1
        % find subset of the raw trajectory where section id == i
        for k = 1:length(list_of_junctionIDs_traj_1)
            if list_of_junctionIDs_traj_1(k) > 0
                temp_raw_trajectory = ...
                    raw_trajectory((raw_trajectory{:,{'junction_id'}} == list_of_junctionIDs_traj_1(k)),:);
                vehicle_path_temp = temp_raw_trajectory{:,{'position_front_x','position_front_y'}};

                % find subset of processsed data where section id == i
                centerline_turn_path_temp = centerline_turn_pose((centerline_turn_pose(:,1) == list_of_junctionIDs_traj_1(k)),:);
...
for index_stop = 1:number_of_stops
    ...
    if size(start_stop_trajectory,1) > min_trip_size
        ...
        input_states = [start_stop_trajectory.current_speed(1); 0; 0; ...
            ...
            processed_vehicle_yaw_interp(indices_to_start(index_stop))];
        U = input_states(1);
        ...
        reference_traversal.Yaw = processed_vehicle_yaw_interp(indices_to_start(index_stop):...
            indices_to_stop(index_stop),2); % smooth yaw here, change variable name to variable
        % calculated above
